 ...
 */
    ...
    if ( $posts->have_posts() ) {
      ...
      while($posts->have_posts()) {
        ...
        if ( has_post_thumbnail() ) {
          $child_posts_output .= get_the_post_thumbnail( get_the_ID(),
            array( 25, 25 ),
            array(
              'class' => 'pull-left',
  ...
  class Lcb_Page_Child_Posts_Widget_Extended extends WP_Widget {
    ...
    function widget($args, $instance) {
      ...
      if ( $posts->have_posts() ) {
        $child_posts_output .= '<ul>';
        while($posts->have_posts()) {
          $posts->the_post();
          $child_posts_output .= '<li><a href="' . get_permalink() . '">';
          $child_posts_output .= get_the_title();
          $child_posts_output .= '</a></li>';
        }
        $child_posts_output .= '</ul>';
        echo $child_posts_output;
      }
